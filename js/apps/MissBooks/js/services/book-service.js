...
const QUERYBOOKS_STORAGE_KEY = 'GoogleBooks'
...
function queryBooks() {
    var books = utilService.load(STORAGE_KEY)
...
function saveBook(book, review) {
    let bookToEdit = book;
    if (!bookToEdit.reviews) {
        bookToEdit.reviews = [];
        bookToEdit.reviews.push(review);
    } else {
        bookToEdit.reviews.unshift(review);
    }
    const idx = gBooks.findIndex(currBook => currBook.id === book.id);
    gBooks.splice(idx, 1, bookToEdit);
    utilService.store(STORAGE_KEY, gBooks)
    return Promise.resolve(book)
}

function saveGoogleQueryBooks(res) {
    let formattedRes = []
    let queryRes = res.map(book => {
        book.title = book.volumeInfo.title;
        book.authors = book.volumeInfo.authors;
        book.categories = book.volumeInfo.categories;
        book.pageCount = book.volumeInfo.pageCount;
        book.publishedDate = book.volumeInfo.publishedDate;
        book.thumbnail = book.volumeInfo.imageLinks.thumbnail;
        book.reviews = [];
        book.listPrice = {};
        if (book.saleInfo.saleability === 'NOT_FOR_SALE' || book.saleInfo.saleability === 'FREE') {
            book.listPrice.amount = 10;
            book.listPrice.currencyCode = 'ILS';
        } else {
            book.listPrice.amount = book.saleInfo.listPrice.amount;
            book.listPrice.currencyCode = book.saleInfo.listPrice.currencyCode;
        }

        formattedRes.push(book)
    })
    gQueryResBooks = formattedRes;
    utilService.store(QUERYBOOKS_STORAGE_KEY, gQueryResBooks)
    return gQueryResBooks;
}

function addGoogleBooktoDB(bookId) {
    let googleBook = gQueryResBooks.find(book => book.id === bookId);
...
function getPrevNextBookId(bookId, direction) {
