...
const STORAGE_KEY_API = 'BooksApi'
var feedbacks = []
const API_BOOK = 'https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%20javascript'
...
window.getBooksApi = getBooksApi()
window.getBooksApi = saveBooksApi()

function getBookById(bookId) {
  const books = gBooks.find(book => book.id === bookId)
...
function addFeedback(bookId, feedback) {
  ...
  return getFeedbacks(bookId)
    .then(list => {
      ...
      storageService.saveToStorage(key, feedbacks)
...
function getFeedbacks(bookId) {
  var feedbacksList = storageService.loadFromStorage(bookId)
  return Promise.resolve(feedbacksList)
}

function deleteFeedback(bookId, idx) {
  var feedbacks = storageService.loadFromStorage(bookId)
...
function saveBooksApi() {
  var booksApi = storageService.loadFromStorage(STORAGE_KEY_API)
  if (!booksApi || booksApi.length === 0) {
    // console.log('11');

    getBooksApi().then(res =>
      storageService.saveToStorage(STORAGE_KEY_API, res.data.items)
    )

  }
  // console.log(booksApi);

  storageService.saveToStorage(STORAGE_KEY_API, booksApi)
  return booksApi
}
...
function addBook(book) {
  ...
  let newBook = {
    id: book.id,
    title: book.volumeInfo.title || 'lorem bla bla ',
    subtitle: book.volumeInfo.subtitle|| 'lorem bla bla ',
    authors: [book.volumeInfo.authors],
    publishedDate: book.volumeInfo.publishedDate,
    description: book.volumeInfo.description || 'lorem bla bla ',
    pageCount: book.volumeInfo.pageCount|| 150,
    categories: [book.volumeInfo.categories],
    thumbnail: book.volumeInfo.imageLinks.thumbnail,
    language: book.volumeInfo.language,
    listPrice: {
      // amount: book.saleInfo.listPrice.amount = '200',
      // currencyCode: book.saleInfo.listPrice.currencyCode || 'dad',
